 ...
 */
public class LatencyGenerator<T extends ScpDeploymentComponent> {

	public static <T> void generate(List<T> scpComponents, boolean verbose) {
		
		for (int i = 0; i < scpComponents.size(); i++) {
			ScpDeploymentComponent c1 = (ScpDeploymentComponent) scpComponents.get(i);
			...
			for (int j = 0; j < i; j++){
				...
				if (i != j){
					ScpDeploymentComponent c2 = (ScpDeploymentComponent) scpComponents.get(j);
					Long min, max;
					...
					ScpLatencyData latencyData = new ScpLatencyData(c1.id, c2.id, min, max);
